...
def mobile(cursor, now, keyword, sitename, device, useragent):
    ...
    for i in links:
        ...
        if sitename in str(i):
            ...
            insert = "INSERT INTO rankings (url, keyword, date, rank, device) values ('" + rank + "', '" + keyword + "', '" + now + "', " + position + ", '" + device + "')"
...
def main():
    ...
    re = cursor.execute("SELECT keyword, last_visited FROM keywords")
    ...
    for keyword in keywords:
        if keyword[1] != now:
            ...
            if empty3url(cursor, keyword[0]):
                ...
                update = "UPDATE keywords SET last_visited='"+now+"' WHERE keyword='"+ keyword[0]+"'"
                print(update)
                execute(cursor, update)
